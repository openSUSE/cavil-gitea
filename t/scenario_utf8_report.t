...
# Reports can contain non-ASCII characters (like the em dash Cavil puts into review summaries)
my $utf8_report = "# Legal Report\n\n### Reviewed by tester \x{2014} AI-assisted \x{2014} tags: review\n";

get '/package/1/report' => [format => 'txt'] => {text => $utf8_report};
...
subtest 'Report with non-ASCII characters' => sub {
  ...
  subtest 'Gitea state' => sub {
    ...
    is_deeply $posted_attachments[0],
      {id => 3, name => 'report.md', filename => 'report.md', content => encode('UTF-8', $utf8_report)},
      'attachment posted with UTF-8 encoded report';
